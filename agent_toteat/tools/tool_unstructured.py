...
# Añade cerca del top (junto a imports)
...
# Default local (estilo FAQS_bucket) → tools/model_c (ajústalo si prefieres .cache/)
...
TOP_K_DEFAULT = int(os.getenv("UNSTRUCTURED_TOP_K", "24"))
...
DOCX_OVERLAP_TOKENS = int(os.getenv("DOCX_OVERLAP_TOKENS", "80"))
...
class ToolAdapter:
    def __init__(self) -> None:
        ...
        # Warmup opcional (descarga/carga del modelo una vez)
        if os.getenv("UNSTRUCTURED_WARMUP", "0") in {"1", "true", "yes"}:
            _ = self.embedder.model  # fuerza carga y cache en disco/RAM
...
# --- ADK wrapper: expone un callable 'tool_unstructured' para el agente ---

_adapter_singleton = ToolAdapter()

def tool_unstructured(query: str, scope: str, files: list[str], top_k: int) -> dict:
    if not scope:
        scope = "auto"
    if not files:
        files = []
    ...
    """
    # Normalización defensiva (por si el modelo manda string en vez de lista)
    if files is None:
        files = []
